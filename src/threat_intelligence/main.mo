...
=======
  // Runtime indexes rebuilt from stable storage
...
>>>>>>> Stashed changes
  transient var indicatorIdx : HashMap.HashMap<Text, [Text]> =
    HashMap.fromIter<Text, [Text]>(
      indicatorIdxEntries.vals(),
      indicatorIdxEntries.size(),
      Text.equal,
      Text.hash
    );
...
=======
    ...
    let prev = switch (indicatorIdx.get(key)) {
      case (?ids) ids;
      case null [];
    };

    let b = Buffer.Buffer<Text>(prev.size() + 1);
    ...
    b.add(id);
    indicatorIdx.put(key, Buffer.toArray(b));
>>>>>>> Stashed changes
  };
  ...
  func lookupIndicator(indicator : Text) : ThreatResult {
    ...
    let ids = switch (indicatorIdx.get(key)) {
      case (?i) i;
      case null [];
    };
      ...
      switch (threats.get(id)) {
        case (?e) {
          if (e.isActive) {
            ...
            if (e.lastSeen > lastUpdate) {
              lastUpdate := e.lastSeen;
            };
          };
        };
  ...
  public query func checkUrl(url : Text) : async ThreatResult {
    lookupIndicator(url)
  };

  public query func checkIp(ip : Text) : async ThreatResult {
    lookupIndicator(ip)
  };

  public query func checkDomain(domain : Text) : async ThreatResult {
    lookupIndicator(domain)
  };

  public query func checkHash(hash : Text) : async ThreatResult {
    lookupIndicator(hash)
  };

  public query func checkEmail(email : Text) : async ThreatResult {
    lookupIndicator(email)
  };

  public shared(msg) func submitThreat(
    input : NewThreatInput
  ) : async SubmitResult {

    if (Text.size(input.indicator) < 3) {
      return #err("Indicator too short");
    };

    if (Text.size(input.description) < 10) {
      return #err("Description must be >= 10 characters");
    };
  ...
  public query func getThreat(id : Text) : async ?ThreatEntry {
    threats.get(id)
  };

  public query func listActiveThreats(
    limit : Nat
  ) : async [ThreatEntry] {
    ...
    for ((_, e) in threats.entries()) {
      if (e.isActive and buf.size() < limit) {
        buf.add(e);
      };
    };
  ...
  public shared(msg) func deactivateThreat(
    id : Text
  ) : async VoidResult {
